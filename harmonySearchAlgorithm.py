...
def algorithm(
    ...
    harmony_memory = HarmonyMemory(num_of_rows_hm, num_of_cols_hm)
    iteration = 1
    ...
    while iteration < num_of_iterations:
        ...
        for note_index in range(0, len(harmony_memory[1])):
            ...
            if hm_considering_rate > random():
                ...
                if hm_pitch_adjusting_rate > random():
                    tmp = new_note + hm_bandwidth * (random() - 0.5) * 1
                    # check if temp is in range
                    while not 0 < tmp < 1:
                        tmp = new_note + hm_bandwidth * (random() - 0.5) * 1
                    new_note = tmp
            else:
                new_note = Harmony.generate_note()
        ...
        val_of_object_fun = Harmony.calculate_obj_fun(new_harmony)
